...
class PlanHatClient:
    def __init__(
        ...
        pmethod: Mapping[str, Any],
        batch_size: int = 5000,
    ) -> None:
        self.api_token = api_token
        self.endpoint = pobject.get("endpoint")
        self.http_method = pmethod.get("http_method")
    ...
    def _request(self, data: List[Mapping] = None) -> requests.Response:
        url = self._get_base_url()
        headers = {"Content-Type": "application/json", **self._get_auth_headers()}
        response = requests.request(method=self.http_method, url=url, headers=headers, json=data)
        return response
