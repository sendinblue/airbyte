...
import static io.airbyte.integrations.destination.iceberg.IcebergConstants.GCS_HMAC_ACCESS_KEY_ID_CONFIG_KEY;
import static io.airbyte.integrations.destination.iceberg.IcebergConstants.GCS_HMAC_SECRET_KEY_CONFIG_KEY;
...
public class GCSConfig implements StorageConfig {
  ...
  public static GCSConfig fromDestinationConfig(@Nonnull final JsonNode config) {
    ...
    GCSConfigBuilder builder = new GCSConfigBuilder().bucketRegion(bucketRegion);

    String warehouseUri = getProperty(config, GCS_WAREHOUSE_URI_CONFIG_KEY);
    if (isBlank(warehouseUri)) {
      throw new IllegalArgumentException(GCS_WAREHOUSE_URI_CONFIG_KEY + " cannot be null");
    }
    if (!warehouseUri.startsWith("gs://")) {
      throw new IllegalArgumentException(
          GCS_WAREHOUSE_URI_CONFIG_KEY + " must starts with 'gs://'");
    }
    builder.warehouseUri(warehouseUri);

    String endpointStr = "https://storage.googleapis.com";
    String pureEndpoint = removeSchemaSuffix(endpointStr);
    builder.sslEnabled(true);
    builder.endpoint(pureEndpoint);
    builder.endpointWithSchema(endpointStr);

    String accessKeyId = getProperty(config, GCS_HMAC_ACCESS_KEY_ID_CONFIG_KEY);
    String secretAccessKey = getProperty(config, GCS_HMAC_SECRET_KEY_CONFIG_KEY);
  ...
  public void check() {
    ...
    // normalize path
    String prefix = this.warehouseUri.replaceAll("^gs?://.+?/(.+?)/?$", "$1/");
